<template>
  <PieChartGraph :data="pieChartData" v-if="taskDetailStore.isClassification" />
...
import { CLASSIFICATION } from "@/constants/task";
...
watchEffect(() => {
  ...
  if (curTargetDistribution && curTargetDistribution.length) {
    if (taskDetailStore.isClassification) {
      pieChartData.value = Object.entries(countBy(curTargetDistribution)).map(
        (item) => {
          return {
            name: item[0],
            value: item[1],
    ...
    if (taskDetailStore.isClassification) {
      binsCount.value = new Set(data).size;
      if (taskDetailStore.taskType === CLASSIFICATION) {
        binsCount.value = 1;
      }
    }
